...
<title>Powered by Homero Software</title>
...
<div class="post-header">
  <div class="image-and-btn">
    <img src="https://media.themoviedb.org/t/p/w440_and_h660_face/${serieData.poster_path}" class="poster-img" alt="${serieData.name || serieData.title}" />
    <div class="post-header__info"><br>
      <button class="bs-favs" card-id="${serieData.id}" id="add-btn" style="display: none;"></button>
      <button class="delete-btn none-btn" card-id="${serieData.id}" id="remove-btn" style="display: none;"></button>
      <ul>
        <li class="tmdb-rate"><i class="fa-solid fa-star"></i> ${serieData.vote_average.toFixed(1)}</li><br>
        <li class="info-item"><i class="fa-regular fa-calendar"></i><span> ${serieData.first_air_date ? serieData.first_air_date.slice(0, 4) : 'N/A'}</span></li><br>
        <li class="info-item"><i class="fa-solid fa-layer-group"></i> ${serieData.number_of_seasons} Temporada${serieData.number_of_seasons > 1 ? 's' : ''}</li><br>
                ...
                if (respuesta.status === 200) {
                    ...
                    const modalOptions = [
                        userInputA ? `<a href="${userInputA}" class="modal-option" id="option1">
                            <i class="fas fa-bolt fa-beat"></i> Supervideo HD
                        </a>` : '',
                        userInputB ? `<a href="{userInputB}" class="modal-option" id="option2">
                            <i class="fas fa-bolt fa-beat"></i> Filemoooon HD
                        </a>` : '',
                        userInputC ? `<a href="${userInputC}" class="modal-option" id="option3">
                            <i class="fas fa-bolt fa-beat"></i> Streamwish HD
                        </a>` : '',
                        userInputD ? `<a href="${userInputD}" class="modal-option" id="option4">
                            <i class="fas fa-bolt fa-beat"></i> Movearnpre HD
                        </a>` : ''
                    ].filter(Boolean).join('');
  ...
  <div class="post-header">
    ...
    <div class="post-details">
      <ul>
        ...
        <li><i class="fas fa-star-half-alt" style="color: #FFD700;"></i> ${datos.vote_average.toFixed(1)}</li><br>
...
<div class="cast-container">
        ...
        <div id="playback-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-title" id="modal-title">Seleccionar Servidor</div>
                ${modalOptions}
            </div>
        </div>
    </div>
  ...
  <!-- Contenedor para iframe oculto -->
  <div id="hidden-iframe-container" class="hidden-iframe-container"></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const hiddenIframeContainer = document.getElementById('hidden-iframe-container');

  // Mostrar contenido principal después de carga
  setTimeout(() => {
  ...
  // Función para destruir completamente la actividad del modal
  function destroyModalActivity() {
    if (tempIframe) {
      tempIframe.remove();
      tempIframe = null;
    }
  ...
  playButton.addEventListener('click', function(e) {
    e.preventDefault();
    destroyModalActivity();
    modalTitle.textContent = 'Servidores Disponibles';
  ...
  transmitButton.addEventListener('click', function(e) {
    e.preventDefault();
    destroyModalActivity();
    modalTitle.textContent = 'Transmitir';
    modalOptions.forEach(option => {
  ...
  modalOptions.forEach(option => {
    option.addEventListener('click', function(e) {
      ...
      hiddenIframeContainer.appendChild(tempIframe);
      
      // Mostrar loading
      loadingModal.style.display = 'flex';
      
      // Cargar contenido en iframe oculto
      tempIframe.src = url;
  ...
  closeModal.addEventListener('click', destroyModalActivity);

  // Clic fuera del modal para cerrar
  window.addEventListener('click', function(e) {
    if (e.target === modal || e.target === loadingModal) {
      destroyModalActivity();
    }
  });
  ...
  // Prevenir recarga al volver atrás
  history.pushState({ modalOpen: true }, '');

  // Manejar carga desde caché
